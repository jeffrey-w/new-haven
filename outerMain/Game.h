...
public:
	...
	// Shuffles this Game's Decks, deals HarvestTiles and Buildings to each Player in this Game's
	// Roster, and intitializes this Game's BuildingPool.
	void setup();

	// TODO need to throw exception if setup has not been called for each of the following:

	// Rotates the selected HarvestTile in the current Player's HarvestTileHand. Throws an
	// exception if the specified selection is not consistent with the current Player's hand.
	void rotateTile(int);
	...
	// the current Player's Hand; or if the specified square is not on this Game's GBMap or it is
	// already occupied.
	...
	// the specified square is not on this Game's GBMap or it is already occupied; or the specified
	// type is not a valid ResourceType.
	...
	// Induces the current Player to yield the remaining resourers they gathered to the other
