...
class TokenGraph {
  ...
  public:
    ...
    // Adds an edge incident on the Nodes at the specified coordinates of this TokenGraph. Throws an exception if either
    // of the specified coordinates does not reference a Node of this TokenGraph.
    void addEdge(std::pair<int, int>, std::pair<int, int>);
    // Returns true iff no Token occupies this TokenGraph at the specified coordinate. Throws an Exception if the
    ...
    size_t emptyNodes() const;
    // Returns the Token occupying this TokenGraph at the specified coordinate, or null if the Node is empty. Throws an
    // exception if the specified coordinate does not referecnce a Node of this TokenGraph.
    ...
    // Returns true iff any of the Nodes of this TokenGraph that are adjacent to the one at the specified coordinate
    // contain a Token of the specified type. Throws an exception if the specified coordinate does not reference a Node
    // of this TokenGraph.
    bool adjacentHolds(std::pair<int, int>, int) const;
    // Places the specified Token into the Node at the specified coordinate of this TokenGraph. Throws an exception if
    // the specified coordinate does not reference a Node of this TokenGraph.
    void setTokenAt(AbstractToken*, std::pair<int, int>);
    // Removes the Token occupying the node at the specified coordinate of this. Throws an exception if the specifeid
  ...
  private:
    struct Node {
        ...
        ~Node();
        // Initializes this Node with the specified Token. Should the type of the specified Token match that of the
        // other, the Node that carries it will be reachable from a search initiated from a Node carrying a Token of
        // that type.
